...
import {connect,sleep} from '../tests/cdp.js';
const minutes=Number(process.argv[2]||30),c=await connect(),ev=c.evaluate,samples=[],started=Date.now();
try{
 await ev(`(async()=>{const m=await import(document.querySelector('script[type=module]').src);window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.start('creative',true);g.flying=true;g.state.time=100;g.mobs=[];ui.render();const {applyQualityPreset}=await import('/src/settings.js?v=31');applyQualityPreset(m.settings,'low');v.applySettings();window.soakEdits=[];})()`);
