...
for(const directory of ['src','tests','scripts'])for(const name of await readdir(new URL(directory+'/',root))){
  if(!/\.m?js$/.test(name))continue;const url=new URL(directory+'/'+name,root),source=await readFile(url,'utf8');
...
const index=new URL('index.html',root),html=await readFile(index,'utf8');await writeFile(index,html.replace(/(\.\/(?:src\/main\.js|src\/natural\.css|src\/multiplayer\.css|src\/multiplayer-modes\.css|src\/field-goals\.css|src\/parkour\.css|src\/polish-ui\.css|style\.css|favicon\.svg))(?:\?v=[a-z0-9.-]+)?/g,'$1?v='+release));
