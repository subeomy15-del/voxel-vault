...
export function summonDragon(g){
 ...
 for(const[x,z]of DRAGON_TOWERS)g.world.set(x,25,z,'dragon_crystal');
 s.kit=true;
...
export function defeatDragon(g,m){
 ...
 if(!s.defeated){s.defeated=true;g.add('dragon_egg');g.add('moonstone',16);g.add('diamond',10);g.add('forge_seal');}
 g.renderer.burst(m.x,m.y+2,m.z,'#bda6ed',70);g.audio.play('reward');g.toast('ENDER DRAGON DEFEATED',s.wins===1?'Dragon Egg +16 Moonstone +10 Diamonds. Take your trophy home!':'Dragon defeated again. Your first victory trophy is saved.','reward');g.emit('dragonDefeated');g.save();
}
export function updateDragon(g,m,dt,launchBolt){
 ...
 const duration={circle:7,breath:4,swoop:4,perch:5},next={circle:'breath',breath:'swoop',swoop:'perch',perch:'circle'};
 ...
 if(m.stage==='breath'){y=27;m.angle=Math.atan2(g.pos.x-m.x,g.pos.z-m.z);
  if(m.cooldown<=0){const origin={x:m.x,y:m.y+1.5,z:m.z};launchBolt(g,origin,{x:g.pos.x-m.x,y:g.pos.y+.8-origin.y,z:g.pos.z-m.z},5,true,{speed:16,color:'#b68ae5'});m.cooldown=m.phase===2?.45:.75;g.audio.play('shoot');}
 }
 ...
 if(m.stage==='perch'){x=0;z=5;y=19;m.angle=Math.atan2(g.pos.x-m.x,g.pos.z-m.z);
  if(m.cooldown<=0){g.slam={x:m.x,y:19,z:m.z,radius:5,time:1.1};m.cooldown=3.3;g.emit('warning',{text:'TAIL SLAM — STEP BACK'});}
 }
