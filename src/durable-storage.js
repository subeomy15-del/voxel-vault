import {slotKey} from './save.js?v=32';
import {saveHealth,setSaveHealth,storageFailure} from './save-health.js?v=32';
...
export class DurableStorage {
  ...
  async replaceState(state){
    await this.flush();const {EditMap}=await import('./edit-map.js?v=32');const world={edits:new EditMap(state.edits)};
...
export async function openDurableStorage(local){
  const worker=new Worker(new URL('./save-worker.js?v=32',import.meta.url),{type:'module'}),storage=new DurableStorage(local,worker);
