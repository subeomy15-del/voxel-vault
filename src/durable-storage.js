import {slotKey} from './save.js?v=37';
import {saveHealth,setSaveHealth,storageFailure} from './save-health.js?v=37';
...
export class DurableStorage {
  ...
  async replaceState(state){
    await this.flush();const {EditMap}=await import('./edit-map.js?v=37');const world={edits:new EditMap(state.edits)};
...
export async function openDurableStorage(local){
  const worker=new Worker(new URL('./save-worker.js?v=37',import.meta.url),{type:'module'}),storage=new DurableStorage(local,worker);
