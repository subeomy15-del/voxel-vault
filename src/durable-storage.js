import {slotKey} from './save.js?v=38';
import {saveHealth,setSaveHealth,storageFailure} from './save-health.js?v=38';
...
export class DurableStorage {
  ...
  queueWorld(state,world,onStatus){
    ...
    const meta=metadata(state),job={key,meta,world,snapshot:world.edits.snapshot(),realms:state.realms,onStatus};
    this.cache.set(key,{state,world});this.desired.get(key+':'+state.dimension)?.snapshot.release();this.desired.delete(key+':'+state.dimension);this.desired.set(key+':'+state.dimension,job);
  ...
  async write(job){
    const {key,meta,world,snapshot,onStatus}=job,id=++this.serial,full=!this.known.has(world);let mainMs=0,revision=snapshot.revision;
    ...
    try{
      ...
      if(full){await batch(meta.dimension,snapshot,true);for(const[d,realm]of Object.entries(job.realms||{}))await batch('realm:'+d,realm.edits||[],true);}
      if(!full&&snapshot.pending.length)await batch(meta.dimension,snapshot.pending);
    ...
    }catch(error){this.known.delete(world);setSaveHealth(this,key,{status:storageFailure(error),message:error.message,durationMs:mainMs});onStatus?.(saveHealth(this,key));return false;}finally{snapshot.release();}
  }
  ...
  async replaceState(state){
    await this.flush();const {EditMap}=await import('./edit-map.js?v=38');const world={edits:new EditMap(state.edits)};
    this.preserveBeforeReplacement(slotKey(state.mode));this.queueWorld(state,world);return this.flush();
  }
  dispose(){for(const job of this.desired.values())job.snapshot.release();this.desired.clear();clearTimeout(this.timer);this.worker.terminate();for(const call of this.calls.values()){clearTimeout(call.timer);call.reject(Error('Save worker closed'));}this.calls.clear();}
}
export async function openDurableStorage(local){
  const worker=new Worker(new URL('./save-worker.js?v=38',import.meta.url),{type:'module'}),storage=new DurableStorage(local,worker);
