export const OUTPOSTS=[
 {id:'lodge',name:'Wayfarer Lodge',dx:18,dz:15,kind:'lodge',color:'#e4bd78',loot:{diamond:2,apple:8,wood:24,iron_ingot:5}},
 {id:'tower',name:'Old Watchtower',dx:-25,dz:-18,kind:'tower',color:'#81c8d7',loot:{diamond:4,arrows:32,moonstone_orb:5}},
 {id:'ruins',name:'Crystal Sanctuary',dx:30,dz:-28,kind:'ruins',color:'#b69be7',loot:{diamond:5,gold_ingot:4,golden_apple:2}}
];
export const FORGE_OFFERS=[]; // Retained only for compatibility with older imports.
export function installOutposts(g){
 ...
 for(const def of OUTPOSTS){
  ...
  if(!site){
   for(let attempt=0;attempt<90;attempt++){
    ...
    if(def.kind==='lodge'){
     ...
     put(-2,0,-2,'stonebrick');put(2,0,-2,'furnace');put(-2,0,2,'bed');put(0,3,2,'lantern');
     ...
     for(let dx=-3;dx<=3;dx++)put(dx,5,-3,'marble');put(0,0,-2,'stonebrick');put(0,1,-2,'violet_crystal');
  ...
  if(!site)continue;
  w.landmarks.push({id:'outpost-'+site.id,name:def.name,subtitle:'Travel supplies and crafting materials',type:'outpost',color:def.color,x:site.x,y:site.y,z:site.z});
