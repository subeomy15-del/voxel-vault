...
try{
  ...
  addEventListener('keydown',e=>{
    ...
    if(e.code==='KeyR')game.dash();if(e.code==='KeyT')game.rotateBuilding();if(e.code==='KeyE')use();if(e.code==='KeyB')game.place(true);if(e.code==='KeyF')game.heal(game.state.inv[game.held]&&ITEMS[game.held]?.kind==='food'?game.held:['bread','roasted_mushroom','carrot','apple'].find(k=>game.state.inv[k]>0)||'apple');if(e.code==='KeyQ')game.heal('potion');
  });
  addEventListener('keyup',e=>{game.keys.delete(e.code);if(e.code==='KeyE')game.placeHeld=false;});
  ...
  addEventListener('pointerup',()=>{game.attackHeld=false;game.placeHeld=false;dragging=false;lastTouch=null;});
  addEventListener('pointercancel',()=>{game.attackHeld=false;game.placeHeld=false;dragging=false;lastTouch=null;});
