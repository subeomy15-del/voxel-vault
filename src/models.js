...
export function toolModel(r,item,name,includeHand=true){
  const g=new THREE.Group(),tier=item.tier||1,metal=!name.startsWith('wood_')&&!name.startsWith('stone_'),material=new THREE.MeshStandardMaterial({color:item.color,roughness:metal?.38:.8,metalness:metal?.45:0});
  ...
  if(item.model==='hammer'){add('#786048',.085,.88,.085,0,-.03,0);add(item.color,.56,.3,.25,0,.38,0);add('#d9e3e6',.065,.33,.28,-.27,.38,0);add('#d9e3e6',.065,.33,.28,.27,.38,0);}
    ...
    const knight=name==='knight_sword',width=knight?.115:tier>=4?.092:.075;
    flat(tier===1?[[-width,.04],[width,.04],[width,.65],[.035,.74],[-.035,.74],[-width,.65]]:tier===2?[[-width,.04],[width,.04],[width,.43],[.035,.46],[.07,.53],[.055,.69],[0,.8],[-.06,.65]]:[[-width,.04],[width,.04],[width*.8,.69],[0,knight?.9:.82],[-width*.8,.69]],knight?.08:.055);
    flat(tier>=4?[[-.24,.08],[-.21,.14],[-.14,.06],[.14,.06],[.21,.14],[.24,.08],[.17,-.015],[-.17,-.015]]:[[-.2,.01],[-.18,.065],[.18,.065],[.2,.01],[.13,-.015],[-.13,-.015]],.11);
    add('#7e6344',.095,.28,.095,0,-.16,0);add(item.color,.13,.065,.12,0,-.32,0);
    add('#e7e5cc',.018,.57,.006,-.035,.34,.033);
    if(tier>=4){add(knight?'#935d45':'#d5dec7',.055,.19,.012,0,.19,.05);add('#d8c487',.095,.07,.12,0,-.015,0);}
  }else{
    ...
    };const head=flat(points[item.kind]);if(tier>=4)head.scale.x=1.14;else if(tier===1)head.scale.x=.9;add('#7d7356',.11,.11,.1,0,.4,0);add('#e2d39f',.045,.04,.009,0,.4,.056);
    if(tier>=3)for(const y of [.18,.28])add(tier>=4?'#dfd8ae':'#768c87',.12,.035,.12,0,y,0);
    if(tier>=5)add('#d6c1e0',.05,.1,.02,.1,.41,.07);
  }
