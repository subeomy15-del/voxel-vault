...
function tryJump(g){
  ...
  if(g.jumpBuffer>0&&(g.grounded||g.coyote>0)){g.velocity=8.8;g.grounded=false;g.coyote=0;g.jumpBuffer=0;g.audio.play('jump');}
}
export function movePlayer(g,dt){
  ...
  g.sprinting=g.moving&&!g.crouching&&!inWater&&g.stamina>3&&(g.keys.has('ShiftLeft')||g.keys.has('ShiftRight'));
  g.stamina=Math.max(0,Math.min(100,g.stamina+dt*(g.sprinting?-9:22)));
  const speed=inWater?3.1:g.dashTime>0?15:g.crouching?2.1:g.sprinting?7.4:4.8;
  if(g.dashTime>0&&!g.moving){f=1;g.moving=true;}
  const acceleration=g.grounded?42:inWater?18:22;
  ...
  if(g.creative&&g.flying){
    ...
    for(let i=0;i<steps;i++){
      ...
      if(g.world.intersects(g.pos.x,y,g.pos.z,height)){
        if(g.velocity<0){g.grounded=true;g.pos.y=Math.round(g.pos.y*1000)/1000;if(g.velocity<-17&&!inWater&&!ladder)g.hurt(Math.floor((-g.velocity-15)*.5));}
        g.velocity=0;break;
      }
      g.pos.y=y;
    }
