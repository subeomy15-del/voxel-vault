...
export class ParticlePool {
  constructor(scene, capacity = 384) {
    ...
    this.alpha = new Float32Array(capacity); this.colors = new Float32Array(capacity * 3);
    this.vectorBuffers = [this.position, this.velocity, this.rotation, this.spin, this.colors]; this.scalarBuffers = [this.life, this.age, this.size];
    ...
    material.customProgramCacheKey = () => 'pooled-voxel-particles-v1';
  ...
  firework(x, y, z, color) {
    if (!this.limit) return;
    this.color.set(color);
    const amount = this.limit < 150 ? 30 : 48;
    for (let i = 0; i < amount; i++) { const a = i * 2.399, t = 1 - 2 * (i + .5) / amount, radius = Math.sqrt(1 - t * t); this.add(x, y, z, Math.cos(a) * radius * 6, t * 6 + 3, Math.sin(a) * radius * 6, 1.4 + Math.random() * .7, .065); }
  }
  ...
  update(dt) {
    ...
    if (this.count) { this.mesh.instanceMatrix.needsUpdate = true; this.mesh.geometry.attributes.particleAlpha.needsUpdate = true; }
