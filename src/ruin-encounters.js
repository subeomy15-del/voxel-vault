...
import {STRUCTURE_TYPES,transform} from './structure-templates.js?v=34';
...
export function tickRuinEncounters(g,dt){
 ...
 for(const site of g.world.ruins.registered.values()){
  ...
  if(distance>42||distance<8||!tier||g.state.opened.includes(site.id))continue;
  const count=Math.min(3,tier+1);
  for(let i=0;i<count;i++){
   const id=site.id+':guard:'+i;if(defeated.has(id)||g.mobs.some(m=>m.ruinGuard===id))continue;
   const a=integerHash(i,7,site.seed)*Math.PI*2,underground=['mine','dungeon','buried'].includes(site.type),[dx,dz]=transform(i%2?2:-2,-2-i,site.rotation,site.mirror),x=underground?site.x+dx+.5:site.x+Math.sin(a)*(site.radius+2),z=underground?site.z+dz+.5:site.z+Math.cos(a)*(site.radius+2),floor=site.y+(site.type==='mine'?-7:-5),y=g.world.ground(x,z,underground?floor+1:undefined);
   ...
   const m=g.spawnMob(x,z,i===0&&tier>=2?'brute':biomeEnemy(g.world,x,y,z,i),y);m.ruinGuard=id;
   if(g.mobs.filter(m=>m.ruinGuard).length>=6)return;
  }
 }
}
