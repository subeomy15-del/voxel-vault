import { World } from './world.js?v=38';
import { meshChunk } from './mesh.js?v=38';
let world,epoch=0;
self.onmessage=({data})=>{
  ...
  if(data.type==='mesh'&&data.epoch===epoch){
    ...
    const geometry=meshChunk(world,data.cx,data.cz,data.underground,{ambientOcclusion:data.ambientOcclusion,localCoordinates:true});
    const transfer=Object.values(geometry).flatMap(g=>Object.values(g).map(a=>a.buffer));
    self.postMessage({type:'mesh',epoch,revision:data.revision,cx:data.cx,cz:data.cz,ambientOcclusion:data.ambientOcclusion,localCoordinates:true,geometry,buildMs:performance.now()-started},transfer);
