...
const ORES={coal:'#a0a6a0',copper:'#dc9971',iron:'#dfd8c6',gold:'#f5d67c',diamond:'#9be7e5',crystal:'#b1f3cc'};
...
export class ViewEffects {
  ...
  texture(name){
    if(!this.textures.has(name)){const svg=icon(name,96).replace('<svg ','<svg xmlns="http://www.w3.org/2000/svg" '),tex=new THREE.TextureLoader().load('data:image/svg+xml;charset=utf-8,'+encodeURIComponent(svg));tex.colorSpace=THREE.SRGBColorSpace;this.textures.set(name,tex);}return this.textures.get(name);
  }
  ...
  update(game,dt){
    ...
    if(this.wing){this.wing.visible=game.gliding&&!game.screen;this.wing.rotation.z=r.settings.bobbing?Math.sin(t*1.7)*.012+(game.keys.has('KeyA')?.035:game.keys.has('KeyD')?-.035:0):0;}
