import {biomeDetail,detailedSurface} from './biome-detail.js?v=38';
import {wetlandPlants} from './wetland-plants.js?v=38';
import {locateStronghold,stampStronghold} from './stronghold.js?v=38';
import {coordinateHash} from './coordinate-hash.js?v=38';
import {continentalClimate,caveRegion} from './terrain-v11.js?v=38';
import {SpatialLights} from './spatial-lights.js?v=38';
import {expandedClimate,regionalClimate,regionalSurface} from './regional-climate.js?v=38';
import {StructureGenerator} from './structure-generator.js?v=38';
import {climateAt,blendedSurface,smooth} from './climate.js?v=38';
import {BIOME_DEFINITIONS} from './biome-registry.js?v=38';
import {EditMap} from './edit-map.js?v=38';
import { netherHeight,netherBlock } from './nether.js?v=38';
import { BLOCKS, BIOMES, LANDMARKS, hash } from './data.js?v=38';
import { canonicalItem } from './resource-map.js?v=38';
import { terrainHeight,treeAt,growTree,plantAt } from './landscape.js?v=38';
import { boxesFor,overlapsBlock,rayShape } from './shapes.js?v=38';
import { CLOUDSTEP,courseGeometry } from './parkour-course.js?v=38';
...
export class World {
  constructor(seed=7821,edits=[],terrain=6,dimension='overworld') {
    ...
    this.stronghold=this.terrain>=11&&dimension==='overworld'?locateStronghold(this):null;this.makeCamp();this.chunkTops=new Map();this.ruins=this.terrain>=7&&dimension==='overworld'?new StructureGenerator(this):null;
  }
  hash(x,z,seed=this.seed){return this.terrain>=11?coordinateHash(x,z,seed+({nether:1711,ender:2917}[this.dimension]||0)):hash(x,z,seed);}
  noise(x,z,scale){const a=Math.floor(x/scale),b=Math.floor(z/scale);let u=x/scale-a,v=z/scale-b;u=u*u*(3-2*u);v=v*v*(3-2*v);return (this.hash(a,b,this.seed)*(1-u)+this.hash(a+1,b,this.seed)*u)*(1-v)+(this.hash(a,b+1,this.seed)*(1-u)+this.hash(a+1,b+1,this.seed)*u)*v;}
  ...
  column(x,z){
    ...
    if(this.dimension==='ender'){
      ...
      const central=gx===0&&gz===0,radius=central?24:15+this.hash(gx,gz,this.seed)*5;
      const angle=Math.atan2(dz,dx),d=Math.hypot(dx,dz),edge=radius+(central?1.2:2.8)*Math.sin(angle*3+this.hash(gx,gz,this.seed)*6)+Math.sin(angle*5)*1.2,exists=d<edge;
      const h=exists?(central?18:18+Math.floor(this.hash(gz,gx,this.seed)*4))+(central?0:Math.floor(this.noise(x,z,12)*2))+Math.floor(Math.max(0,d-9)*.09):-64;
    ...
    const region=this.terrain>=7?smooth(280,540,Math.hypot(x,z)):0,climate=region>0?(this.terrain>=11?continentalClimate:this.terrain>=10?expandedClimate:this.terrain>=8?regionalClimate:climateAt)(this.seed,x,z):null;
    ...
    if(this.terrain>=10){
      const gx=Math.floor(x/128),gz=Math.floor(z/128),fx=x-(gx*128+64+((this.hash(gx,gz,this.seed+611)-.5)*48)),fz=z-(gz*128+64+((this.hash(gz,gx,this.seed+617)-.5)*48)),d=Math.hypot(fx,fz),feature=this.hash(gx,gz,this.seed+623);
    ...
    const wrap=(n,span)=>((n+span/2)%span+span)%span-span/2,cx=Math.floor(x/52),cz=Math.floor(z/52),px=cx*52+20+this.hash(cx,cz,this.seed)*12,pz=cz*52+22;const v={h,biome,climate,region,cave:this.terrain>=11?caveRegion(this.seed,x,z):null,t1:-16+Math.sin(z*.047)*5,t2:-34+Math.sin(x*.039)*7,river,a2:(wrap(x-Math.sin(z*.033)*17,80)/3.2)**2,b2:(wrap(z-Math.sin(x*.035)*19,88)/4.4)**2,chamber:((x-px)/13)**2+((z-pz)/16)**2,cy:-22-this.hash(cz,cx,this.seed+7)*23,rock:this.noise(x+9,z-31,28)>.7?'granite':this.noise(x-67,z+84,34)>.64?'limestone':'stone'};this.columns.set(key,v);return v;
  }
  ...
  caveNode(cx,cz){
    const ox=(this.hash(cx,cz,this.seed+701)-.5)*22,oz=(this.hash(cz,cx,this.seed+709)-.5)*22;
    return {x:cx*64+32+ox,z:cz*64+32+oz,y:-20-Math.floor(this.hash(cx,cz,this.seed+719)*30)};
  }
  caveNetwork(x,y,z){
  ...
  base(x,y,z,column=null){
    ...
    if(this.dimension==='ender'){
      ...
      const spireHeight=c.central?8:6+Math.floor(this.hash(Math.round(x/48),Math.round(z/48),this.seed)*6);
      ...
      return this.hash(Math.floor(x/3)+Math.floor(y/3)*127,Math.floor(z/3),this.seed+52)>.6&&this.hash(x+y*127,z,this.seed)<.07?'moonstone':y<c.bottom+2?'obsidian':'end_stone';
    }
    ...
    if(this.terrain>=11&&c.biome==='marsh'&&y===h)return h<=5?'mud':'grass';
    if(this.terrain<11&&c.biome==='marsh'&&y===h&&this.noise(x+91,z,12)>.58)return 'water';
    if(y===h&&c.region>.5&&this.terrain>=8){const surface=regionalSurface({...c.climate,h,biome:c.biome},x,z,this.seed);return this.terrain>=12?detailedSurface(this,x,z,c,surface):surface;}
    ...
    const rock=this.terrain>=11&&y<h-4?c.cave.rock:y<-49?'basalt':y<-38?'slate':c.rock==='limestone'&&y<-5?'marble':c.rock;
    if(y>h-8)return rock;
    // Veins occur only beneath a substantial layer of soil/rock.
    const vein=this.hash(Math.floor(x/3)+Math.floor(y/3)*127,Math.floor(z/3),this.seed+93),fleck=this.hash(x+y*117,z-y*43,this.seed);
    // Broad barren patches separate smaller, broken veins. The seed keeps them stable.
    const region=this.hash(Math.floor(x/24)+Math.floor(y/18)*73,Math.floor(z/24),this.seed+517);
    if(region>.35&&fleck<.55){if(this.terrain>=11&&vein>.7&&y<-35)return 'obsidian';if(vein<.025&&y<-30)return 'diamond';if(vein<.05&&y<-18)return 'gold';if(vein<.125)return 'iron';if(vein<.19)return 'coal';}
    return rock;
  }
  ...
  waterAt(x,y,z){const type=this.get(Math.floor(x),Math.floor(y),Math.floor(z));return type==='water'||!!BLOCKS[type]?.waterlogged;}
  set(x,y,z,type){
  ...
  prepare(cx,cz){if(this.dimension!=='overworld')return;
    ...
    this.ruins?.stamp(cx,cz,sites,stamp);stampStronghold(this,cx,cz,stamp);
    const put=(x,y,z,type)=>{if(Math.floor(x/16)!==cx||Math.floor(z/16)!==cz)return;const key=cellKey(x,y,z),old=this.structures.get(key);if(reserved.has(key))return;if(!this.structures.has(key)||['leaf','pine','autumnleaf','cherry_leaf','jungle_leaf','mangrove_leaf'].includes(old)||this.terrain>=12&&old&&BLOCKS[old]&&!BLOCKS[old].solid){this.structures.set(key,type);this.chunkTops.set(tag,Math.max(this.chunkTops.get(tag)||0,y+2));}};
    ...
    for(let x=cx*16-margin;x<cx*16+16+margin;x++)for(let z=cz*16-margin;z<cz*16+16+margin;z++){
      ...
      if(safe||this.stronghold&&Math.abs(x-this.stronghold.x)<28&&Math.abs(z-this.stronghold.z)<28||c.h>(this.terrain>=7?70:58)||this.ruins?.reserved(x,z,sites,3))continue;
      if(this.terrain>=11)wetlandPlants(this,put,x,z,c);
      if(c.h<(this.terrain>=8?SEA_LEVEL+1:SEA_LEVEL))continue;
      if(this.terrain>=12)biomeDetail(this,put,x,z,c);
      if(c.h<=SEA_LEVEL+2&&c.river<14){if(this.hash(x,z,this.seed+32)>.982)put(x,c.h+1,z,'cane');continue;}
      if(c.biome==='desert'||c.biome==='badlands'){if(this.hash(x,z,this.seed+32)>.997)for(let y=1;y<=2+Math.floor(this.hash(z,x,this.seed)*2);y++)put(x,c.h+y,z,'cactus');continue;}
      if(this.terrain<11&&c.biome==='marsh'&&this.noise(x+91,z,12)>.58){if(this.hash(x,z,this.seed+32)>.97)put(x,c.h+1,z,'cane');continue;}
  ...
  baseSurface(x,z){const c=this.column(x,z);return this.terrain<11&&c.biome==='marsh'&&this.noise(x+91,z,12)>.58?'water':c.h<SEA_LEVEL?'water':'land';}
  biomeAtHeight(x,y,z){return this.dimension==='overworld'&&y<this.height(x,z)-6?(this.terrain>=11?this.column(Math.floor(x),Math.floor(z)).cave.id:y<-28?'deep_cave':'cave'):this.biome(x,z);}
  prepareLegacy(cx,cz){
    const tag=`${cx},${cz}`;if(this.prepared.has(tag))return;this.prepared.add(tag);
    ...
    for(let x=cx*16-3;x<cx*16+19;x++)for(let z=cz*16-3;z<cz*16+19;z++){
      const c=this.column(x,z),n=this.hash(x,z,this.seed+32),camp=Math.hypot(x,z-14)<9,entrance=x>16&&x<29&&z>-38&&z<16;
      ...
      if(n<threshold){
        const flora=this.hash(x,z,this.seed+218),patch=this.noise(x+312,z-119,28);
        if(c.biome!=='snow'&&c.biome!=='mountain'&&patch>.63&&flora>.952){
          ...
          const pick=this.hash(Math.floor(x/24),Math.floor(z/24),this.seed+371),type=plants[Math.min(plants.length-1,Math.floor(pick*plants.length))];
          put(x,c.h+1,z,type);
        }
        continue;
      }
      const birch=this.hash(z,x,this.seed+4)>.65,wood=c.biome==='snow'?'pinewood':birch?'birch':'wood',leaf=c.biome==='snow'?'pine':'leaf',tall=5+Math.floor(this.hash(x,z,this.seed+65)*3);
