...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=31');window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.state=(await import('/src/save.js?v=31')).freshState(72);g.loadWorld();g.pos={x:240.5,y:g.world.height(240,180)+1,z:180.5};g.state.time=110;g.screen='pause';ui.settings.perspective=0;ui.render();})()`);await ready();
 ...
 await c.send('Page.reload');await sleep(1200);await ev(`(async()=>{const m=await import('/src/main.js?v=31');window.g=m.game;window.ui=m.ui;g.start();g.pause('inventory');ui.render();})()`);
