...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=20');window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.state=(await import('/src/save.js?v=20')).freshState(7821);g.loadWorld();g.pos={x:-70.5,y:g.world.ground(-70.5,24.5),z:24.5};g.screen=null;g.yaw=-.7;g.pitch=.08;ui.render();})()`);await ready();await c.screenshot('cool-overworld');
 ...
 await ev('g.save()');await c.send('Page.reload');await sleep(1000);await ev(`(async()=>{const m=await import('/src/main.js?v=20');window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.screen=null;ui.render()})()`);assert.equal(await ev('g.boss?.kind'),'dragon');
