...
import {freshState,saveState,loadState} from '../src/save.js?v=13';
import {summonDragon,DRAGON_TOWERS} from '../src/dragon.js?v=13';
import {launchBolt,updateProjectiles} from '../src/combat.js?v=13';
...
test('coal only fuels furnaces and current recipes never require retired ores',()=>{
 ...
 for(const r of RECIPES){assert.ok(!ITEMS[r.item].hidden,r.item);for(const key of Object.keys(r.cost))assert.ok(!['coal','copper','copper_ingot','crystal','ruby','emerald','sapphire'].includes(key),r.item+': '+key);}
