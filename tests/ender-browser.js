...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=10');window.g=m.game;window.v=m.renderer;window.ui=m.ui;ui.settings.perspective=0;})()`);
 if(process.argv.includes('--previews')){
  ...
  for(const [name,mode,x,z]of [['survival','adventure',-70,24],['creative','creative',0,14],['daily','daily',-135,-78],['ender','ender',0,0]]){
   await ev(`(async()=>{g.state=(await import('/src/save.js?v=10')).freshState(7821,'${mode}'==='ender'?'adventure':'${mode}');g.loadWorld();if('${mode}'==='ender')g.travel('ender');g.pos={x:${x},y:g.world.height(${x},${z})+1,z:${z}};g.state.time=110;g.screen='menu';ui.render();})()`);
 ...
 await c.send('Page.reload');await sleep(700);await ev(`(async()=>{const m=await import('/src/main.js?v=10');window.g=m.game;window.v=m.renderer;window.ui=m.ui;})()`);await c.click('[data-action="ender"]');await ready();assert.equal(await ev('g.world.get(2,27,2)'),'moonstone_block');assert.equal(await ev('ui.settings.perspective'),1);
