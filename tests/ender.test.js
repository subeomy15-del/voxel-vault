...
import { ITEMS,RECIPES,craft } from '../src/data.js?v=10';
import { cameraPosition } from '../src/perspective.js?v=10';
...
test('Ender islands have a safe arrival, real voids, resources and visible undersides',()=>{
 const w=new World(72,[],6,'ender'),p=w.findSpawn();assert.equal(w.intersects(p.x,p.y,p.z),false);assert.equal(w.get(0,19,0),'ender_gate');assert.equal(w.get(24,18,24),null);assert.equal(w.get(5,26,-4),'violet_crystal');
...
test('moonstone orb spends one item only for a safe landing and never crosses a wall',()=>{
 const g=game();g.start('ender');g.select(4);g.pos={x:.5,y:19,z:8.5};g.yaw=0;g.pitch=0;const count=g.state.inv.moonstone_orb;assert.equal(g.useOrb(),true);assert.equal(g.state.inv.moonstone_orb,count-1);assert.equal(g.world.intersects(g.pos.x,g.pos.y,g.pos.z),false);
...
test('realm travel carries the same inventory, health and equipment and persists both sides on reload',()=>{
 const g=game();g.start('adventure');const home={...g.pos};g.add('diamond',13);g.state.hp=14;g.world.set(3,44,3,'gold_block');g.save();g.travel('ender');assert.equal(g.state.mode,'adventure');assert.equal(g.state.inv.diamond,13);assert.equal(g.state.hp,14);g.add('moonstone',8);g.add('armor');g.state.armor='armor';g.world.set(4,36,4,'moonstone_block');g.save();
...
test('Rift Run requires reaching each anchor, rewards once, and saves completion',async()=>{
 const {RIFT_ANCHORS}=await import('../src/realms.js?v=10');const g=game();g.start('ender');assert.equal(g.collectAnchor('dawn'),false);g.state.elapsed+=20;
...
test('the first island is reachable by ordinary movement and the starter glider across seeds',()=>{
 for(const seed of [7,72,7821,917,381]){
  const g=game();g.state.seed=seed;g.travel('ender');g.pos={x:.5,y:19,z:-9.5};g.grounded=true;g.yaw=0;g.pitch=-.1;g.keys.add('KeyW');let reached=false;
...
test('all three anchor islands can be reached with launch pads, steering and closing the starter glider',()=>{
 for(const seed of [7,72,7821,917,381]){
  const g=game();g.state.seed=seed;g.travel('ender');
