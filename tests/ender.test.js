...
import { World } from '../src/world.js?v=23';
import { Game } from '../src/game.js?v=23';
import { freshState,loadState,saveState } from '../src/save.js?v=23';
import { ITEMS,RECIPES,craft } from '../src/data.js?v=23';
import { cameraPosition } from '../src/perspective.js?v=23';
import { meshChunk } from '../src/mesh.js?v=23';
...
test('Rift Run requires reaching each anchor, rewards once, and saves completion',async()=>{
 const {RIFT_ANCHORS}=await import('../src/realms.js?v=23');const g=game();g.start('adventure');g.state.fortressCleared=true;g.save();g.start('ender');g.add('hang_glider');g.state.glider='hang_glider';g.add('moonstone_orb',12);assert.equal(g.collectAnchor('dawn'),false);g.state.elapsed+=20;
