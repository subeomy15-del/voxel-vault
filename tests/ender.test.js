...
import { cameraPosition } from '../src/perspective.js?v=37';
...
test('Rift Run requires reaching each anchor, rewards once, and saves completion',async()=>{
 const {RIFT_ANCHORS}=await import('../src/realms.js?v=37');const g=game();g.start('adventure');g.state.fortressCleared=true;g.save();g.start('ender');g.add('hang_glider');g.state.glider='hang_glider';g.add('moonstone_orb',12);assert.equal(g.collectAnchor('dawn'),false);g.state.elapsed+=20;
