...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=36');window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.state=(await import('/src/save.js?v=36')).freshState(7821);g.loadWorld();g.screen=null;ui.settings.perspective=0;ui.render();window.site=g.state.outposts.find(p=>p.id==='lodge');g.pos={x:site.x+.5,y:g.world.ground(site.x+.5,site.z+8.5),z:site.z+8.5};g.yaw=0;g.pitch=0;})()`);await ready();await c.screenshot('quality-lodge');
