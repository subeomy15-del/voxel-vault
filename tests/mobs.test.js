...
import {ENEMIES} from '../src/combat.js?v=34';
...
test('new realm mobs have distinct roles and can spawn',()=>{
 ...
 const g=make();g.start();for(const kind of ['enderling','void_archer','frost_howler']){const m=g.spawnMob(3,3,kind,6);assert.equal(m.kind,kind);assert.equal(m.hp,Math.ceil(ENEMIES[kind].hp*1.4));}
});
