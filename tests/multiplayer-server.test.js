...
test('HTTP accepts allowed origins, validates bodies and serves only browser assets', async t => {
  ...
  for (const path of ['/.git/config', '/package.json', '/server.js', '/server/lobbies.js', '/tests/multiplayer-server.test.js', '/assets/../../server.js', '/src/%2e%2e/server.js?v=35']) assert.equal((await fetch(api.base + path)).status, 404, path);
  for (const path of ['/', '/src/main.js?v=35', '/style.css', '/favicon.svg', '/vendor/three.module.js']) assert.equal((await fetch(api.base + path)).status, 200, path);
});
