...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=37');window.g=m.game;window.v=m.renderer;window.ui=m.ui;ui.settings.perspective=0;window.hideHUD=document.createElement('style');hideHUD.textContent='body>*:not(#world){visibility:hidden!important}';document.head.append(hideHUD);})()`);
 for(const [name,mode,x,z]of [['survival','adventure',-70,24],['creative','creative',0,14],['daily','daily',-135,-78]]){
  await ev(`(async()=>{g.state=(await import('/src/save.js?v=37')).freshState(7821,'${mode}');g.loadWorld();g.pos={x:${x},y:g.world.height(${x},${z})+1,z:${z}};g.state.time=110;g.screen='menu';ui.render();})()`);
 ...
 await writeFile(process.argv.includes('--previews')?new URL('../assets/ender-preview.png',import.meta.url):'/private/tmp/voxel-vault-natural-ender.png',Buffer.from((await c.send('Page.captureScreenshot')).data,'base64'));
 ...
 assert.ok(await ev("document.querySelector('.lobby-logo').textContent.includes('VAULT')"));
