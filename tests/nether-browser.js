...
try{
 ...
 await ev(`(async()=>{const m=await import('/src/main.js?v=34');window.g=m.game;window.ui=m.ui;ui.settings.perspective=0;g.start('adventure',true);g.pause('inventory');ui.render();})()`);
 ...
 await ev("g.target=null;g.pos={x:72.5,y:25,z:-46.5};g.interact();ui.render()");assert.equal(await ev('g.state.dimension'),'nether');await ev("g.updateFortress();g.hit(g.mobs.find(m=>m.fortress),g.mobs.find(m=>m.fortress).maxHp);g.interact();ui.render()");assert.equal(await ev('g.state.dimension'),'ender');
