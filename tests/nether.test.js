...
test('three realm portal journey preserves inventory, terrain and reloads',()=>{
 ...
 g.world.set(10,40,10,'iron_block');g.pos={x:72.5,y:25,z:-40};g.updateFortress();assert.equal(g.travel('ender'),false);const guard=g.mobs.find(m=>m.fortress);assert.ok(guard);g.hit(guard,200);assert.equal(g.state.fortressCleared,true);use({x:72,y:25,z:-48});assert.equal(g.state.dimension,'ender');
