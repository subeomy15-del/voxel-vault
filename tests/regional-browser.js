...
try{
 ...
 await c.evaluate(`(async()=>{const m=await import(document.querySelector('script[type=module]').src);window.g=m.game;window.v=m.renderer;window.ui=m.ui;const {freshState}=await import('/src/save.js?v=36');g.state=freshState(7821,'creative');g.loadWorld();g.screen='pause';g.state.time=110;g.mobs=[];ui.render();const {applyQualityPreset}=await import('/src/settings.js?v=36');applyQualityPreset(m.settings,'medium');v.applySettings(m.settings);const style=document.createElement('style');style.textContent='body>*:not(#world){visibility:hidden!important}';document.head.append(style);})()`);
 const results=[];
 ...
 const parity=await c.evaluate(`(async()=>{const {meshChunk}=await import('/src/mesh.js?v=36');const cx=Math.floor(g.pos.x/16),cz=Math.floor(g.pos.z/16);const sum=m=>{let h=2166136261;for(const g of Object.values(m))for(const a of Object.values(g))for(const b of new Uint8Array(a.buffer))h=Math.imul(h^b,16777619);return h>>>0;};const worker=new Worker('/src/terrain-worker.js?v=36',{type:'module'});const remote=await new Promise((resolve,reject)=>{worker.onmessage=e=>resolve(e.data.geometry);worker.onerror=reject;worker.postMessage({type:'init',epoch:1,seed:g.world.seed,terrain:g.world.terrain,dimension:'overworld',edits:[...g.world.edits]});worker.postMessage({type:'mesh',epoch:1,revision:0,cx,cz,ambientOcclusion:true});});worker.terminate();return [sum(meshChunk(g.world,cx,cz)),sum(remote)];})()`);assert.equal(...parity);
