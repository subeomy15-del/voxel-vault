...
try{
 ...
 await c.evaluate(`(async()=>{
  const main=await import('/src/main.js?v=21'),THREE=await import('/vendor/three.module.js'),{ITEMS}=await import('/src/data.js?v=21'),{itemModel}=await import('/src/item-model.js?v=21');
  ...
  window.renderItem=name=>{const group=itemModel(main.renderer,name);if(['sword','pickaxe','axe','shovel','hoe','grapple'].includes(ITEMS[name].kind))group.rotation.z=-.55;if(ITEMS[name].kind==='glider')group.rotation.x=.8;group.updateMatrixWorld(true);const bounds=new THREE.Box3().setFromObject(group),size=bounds.getSize(new THREE.Vector3()),center=bounds.getCenter(new THREE.Vector3());const holder=new THREE.Group();group.position.sub(center);holder.add(group);holder.scale.setScalar(1.14/Math.max(size.x,size.y,size.z,.1));scene.add(holder);renderer.render(scene,camera);const data=renderer.domElement.toDataURL('image/png').split(',')[1];main.renderer.disposeGroup(holder);return data;};
