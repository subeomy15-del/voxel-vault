...
try{
  ...
  await ev(`(async()=>{const m=await import('./src/main.js?v=33');window.g=m.game;window.v=m.renderer;window.ui=m.ui;g.start('adventure',true);g.pos={x:.5,y:7,z:9.5};g.yaw=0;g.pitch=-.95;g.add('stone_hoe');g.equip('stone_hoe');ui.render();})()`);
  ...
  assert.ok(await ev(`(async()=>{const {ITEMS}=await import('/src/data.js?v=33');return Array.from(document.querySelectorAll('.item-card')).every(e=>{const k=e.dataset.inspect;return (ITEMS[k].name+' '+ITEMS[k].description).toLowerCase().includes('diamond')})})()`));
