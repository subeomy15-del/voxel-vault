...
import { ITEMS,BLOCKS,CROPS,RECIPES,ORE_GLIDERS,canCraft,craft } from '../src/data.js?v=37';
import { ANIMALS,animalKind } from '../src/wildlife.js?v=37';
import { tickSurvival,canEat } from '../src/survival.js?v=37';
import { updateProjectiles } from '../src/combat.js?v=37';
import { loadState } from '../src/save.js?v=37';
...
test('all six huntable species drop their own raw meat and collect it once',()=>{
  for(const[kind,info]of Object.entries(ANIMALS).filter(([kind])=>['deer','pig','cow','sheep','chicken','rabbit'].includes(kind))){
    ...
    for(const[item,[low,high]]of Object.entries(info.drops)){const drop=g.state.drops.find(d=>d.item===item);assert.ok(drop&&drop.count>=low+1&&drop.count<=high+1,kind+' '+item);assert.equal(g.state.inv[item]||0,0);}
