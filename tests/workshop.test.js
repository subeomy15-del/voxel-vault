...
test('stone resists hand mining and the right pickaxe is faster',()=>{
 ...
 g.mineProgress=0;g.state.bar[g.state.selected]='wood_pickaxe';g.mine(1);assert.ok(g.mineProgress>hand*3);assert.ok(g.mineProgress<1);g.mine(5);assert.equal(g.world.get(150,70,150),null);
});
