...
try{
 ...
 await ev(`(async()=>{const m=await import(document.querySelector('script[type=module]').src);window.g=m.game;window.v=m.renderer;window.ui=m.ui;window.settings=m.settings;const {freshState}=await import('/src/save.js?v=38');g.state=freshState(7821,'creative');g.loadWorld();g.flying=true;g.screen=null;g.mobs=[];ui.render();const {applyQualityPreset}=await import('/src/settings.js?v=38');applyQualityPreset(settings,'medium');v.applySettings(settings);})()`);
 const metrics=[];
 ...
 const checksum=await ev(`(async()=>{const {meshChunk}=await import('/src/mesh.js?v=38');const cx=Math.floor(g.pos.x/16),cz=Math.floor(g.pos.z/16),local=meshChunk(g.world,cx,cz);const hash=mesh=>{let n=2166136261;for(const group of Object.values(mesh))for(const values of Object.values(group)){const bytes=new Uint8Array(values.buffer);for(const b of bytes)n=Math.imul(n^b,16777619);}return n>>>0;};const worker=new Worker('/src/terrain-worker.js?v=38',{type:'module'});const remote=await new Promise((resolve,reject)=>{worker.onmessage=e=>resolve(e.data.geometry);worker.onerror=reject;worker.postMessage({type:'init',epoch:1,seed:g.world.seed,terrain:g.world.terrain,dimension:g.world.dimension,edits:[...g.world.edits]});worker.postMessage({type:'mesh',epoch:1,revision:0,cx,cz,underground:false,ambientOcclusion:true});});worker.terminate();return {main:hash(local),worker:hash(remote)};})()`);assert.equal(checksum.main,checksum.worker,'Main and worker chunk geometry must agree');
